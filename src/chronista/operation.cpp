...
Operation::Operation(const std::string operation)
{
    if (!std::regex_match(operation, std::regex("^T[0-9]+: [rw]\\([a-zA-Z0-9]+(\\.[a-zA-Z0-9]+){0,4}\\)$")) && !std::regex_match(operation, std::regex("^T[0-9]+: c$")))
    {
...
int Operation::get_transaction_id()
{
