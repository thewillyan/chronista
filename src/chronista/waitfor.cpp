...
std::vector<unsigned int> WaitForGraph::find_deadlock_nodes() const {
  ...
  bool found_cycle = false, found_unvisited_neighbor = false;
    ...
    if (nodes_info_it->second.status == UNVISITED) {
      ...
      while (dfs_stack.size() > 0 && !found_cycle) {
        ...
        if (!current_adjacency_list->empty()) {
          ...
          while (adjacent_node_it != current_adjacency_list->end() &&
                 !found_cycle) {
            ...
            } else if (!found_unvisited_neighbor &&
                       nodes_info[*adjacent_node_it].status == UNVISITED) {
